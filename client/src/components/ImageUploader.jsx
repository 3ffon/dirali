...
function VideoThumbnail({ image, onDelete, onPlay }) {
  const url = useImageUrl(image.id)
  if (!url) return <div className="image-item image-item--loading" />
...
function DocThumbnail({ image, index, onDelete, onOpen }) {
...
function ImageUploader({ apartmentId, address, images, onUpdate }) {
  ...
  return (
    <div className="image-uploader">
      <div className="category-header">📷 תמונות, סרטונים ומסמכים</div>
      ...
      {videoItems.length > 0 && (
        <div className="image-grid">
          {videoItems.map((img) => (
            <VideoThumbnail key={img.id} image={img} onDelete={handleDelete} onPlay={setActiveVideo} />
          ))}
        </div>
      )}
      ...
      {docItems.length > 0 && (
        <div className="image-grid">
          {docItems.map((img, i) => (
            <DocThumbnail key={img.id} image={img} index={i} onDelete={handleDelete} onOpen={setDocStartIndex} />
          ))}
        </div>
      )}
